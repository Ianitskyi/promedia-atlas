...
export function build(){
 ...
 for(const members of grouped){
  ...
  if(!explicit&&linked.length===1){name=linked[0].name;nameEn=linked[0].nameEn||linked[0].name}
  const newsIds=[id,...linked.map(c=>c.id)];
  const profile={id,name,nameEn,registryIds:ids,newsIds,community:linked[0]||null,grouping:explicit?.reason||'Однакова повна назва та спільна особа або компанія, на яку оформлено медіа. Регістр літер, лапки й зайві пробіли нормалізовано.',media:members,entities:Object.fromEntries(entityIds.map(e=>[e,d.entities[e]])),regions:d.regions,date:d.date};
  profiles.push(profile);members.forEach(m=>index[m.id]={id,name,nameEn,count:members.length});
  directory.push({id,name,nameEn,city:cities.join('; '),registryIds:ids,newsIds,url:'https://atlas.promedia.report/media/'+id,status:'approved'});
 }
