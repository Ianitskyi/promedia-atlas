...
export default function UtilityNav({locale,view,onView,profile=false,mediaId}:{locale:'uk'|'en';view?:string;onView?:(v:string)=>void;profile?:boolean;mediaId?:string}){
 const en=locale==='en';
 const home=en?'https://promedia.report/en':'https://promedia.report';
 const ukPath=mediaId?'/media/'+mediaId:'/';
 const enPath=mediaId?'/en/media/'+mediaId:'/en';
 return <div className="utility-bar">
  ...
  {profile?<a className="nav-link" href={en?'/en':'/'}>{en?'Map and directory':'Карта й каталог'}</a>:<>
   ...
   <button className={'nav-link '+(view==='analytics'?'active':'')} onClick={()=>onView?.('analytics')}>{en?'Analytics':'Аналітика'}</button>
   <button className={'nav-link '+(view==='guide'?'active':'')} onClick={()=>onView?.('guide')}>{en?'How to register media':'Як зареєструвати медіа'}</button>
  </>}
  <a className="nav-link" href="https://news.promedia.report/">{en?'News':'Новини'}</a>
